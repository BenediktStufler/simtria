...
struct hedge *hegen(INT *D, INT n) {
	...
	ex = (struct hedge *) calloc(6*n, sizeof(struct hedge));
...
void closure(struct hedge *ex, struct hedge *x, struct hedge *y, int swi) {
	...
	do {
		if(cur->op == cur->ne) {		// current half-edge points to a leaf
			...
			// vertex. hence we need to check if the two after that
			// point to inner vertices as well.
			if( !(cur->ne->ne->op == cur->ne->ne->ne || cur->ne->ne->ne->op == cur->ne->ne->ne->ne) ) {
	...
	// find C
	while( !(cur->pr->op==cur->pr->ne && cur->ne->op==cur->ne->ne) )	
		cur = cur->ne;
	c1 = cur;
...
// add two leaves at each vertex with uniform random locations
INT *enrich(INT *D, INT n, gsl_rng *rgen) {
	...
	// select 2 random distinct boxes out of D[0]+2 boxes
	a = gsl_rng_uniform_int(rgen, D[0]+2);
	b = gsl_rng_uniform_int(rgen, D[0]+1);
	if(b>=a) b++;
	out[0] = D[0]+2;
	for(i=0;i<D[0]+2;i++) {
		if( (i!=a) && (i!=b) ) {
	...
	while(uqueue) {
		if(spop(&uqueue)) {
			...
			//add outdegree with 2 extra leaves
			out[pout] = D[pin]+2;

			// choose locations of leaves
			a = gsl_rng_uniform_int(rgen, D[pin]+2);
			b = gsl_rng_uniform_int(rgen, D[pin]+1);
			if(b>=a) b++;
...
/*
 * Simulate a simple triangulation with a given number of triangles
 */
int simtria(struct cmdarg *comarg, gsl_rng **rgens) {
