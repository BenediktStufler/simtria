...
struct cmdarg
{
	...
	INT size;			// target size of random triangulation
...
{
	{"size",		's', "SIZE", 0, 	"Generate a uniform planted simple triangulation with s >= 4 faces, 2 + s/2 vertices, and 3s/2 edges. Has to be divisible by two and has to be greater than or equal to four."},
...
static error_t parse_opt(int key, char *arg, struct argp_state *state) {
	...
	switch (key) {
		...
		case 's':
			...
			if(arguments->size < 4) {
				fprintf(stderr, "Error: the SIZE command line argument must be at least four.\n");
				exit(-1);
			}
			if(arguments->size %2 != 0) {
				fprintf(stderr, "Error: the SIZE command line argument needs to be divisible by two.\n");
				exit(-1);
			}
			break;
		case 'S':
...
*/
static char doc[] = "simtria -- generate uniform simple triangulations";
